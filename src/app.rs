...
pub enum AppControl {
    ...
    SendControl(usize, CommandControl),
}

pub struct App {
    pub pane_manager: PaneManager,
    pub tasks: HashMap<usize, Command>,
    pub input_state: InputState,
    pub exit: bool,
    pub output_rx: mpsc::Receiver<(usize, String)>,
    pub output_tx: mpsc::Sender<(usize, String)>,
...
impl App {
    ...
    pub async fn set_command(&mut self, id: usize, exec: String) {
        ...
        let task_handle = tokio::spawn(async move {
            Command::run_command_task(id, cmd, interval, control_rx, output_tx_clone).await;
        });
    ...
    pub fn load_session(
        ...
        tasks_state: HashMap<usize, CommandSerializableState>,
    ) -> io::Result<()> {
        ...
        for (id, state) in tasks_state.into_iter() {
            ...
            let task_handle = tokio::spawn(async move {
                Command::run_command_task(id, cmd_exec, interval, control_rx, output_tx_clone)
                    .await;
            });
...
mod tests {
    ...
    fn test_app_new_initializes_correctly() {
        ...
        assert_eq!(app.pane_manager.get_active_pane_id(), 1);
    }
    ...
    async fn test_set_command_adds_new_task() {
        ...
        app.set_command(id, exec.to_string()).await;

        assert_eq!(app.tasks.len(), 1);
        let command = app.tasks.get(&id).unwrap();
